...
},{"./js/LocalStorage":"DkDak","./js/signIn":"lQpPB","./js/signUp":"5WLbD"}],"DkDak":[function(require,module,exports) {
...
const getUserFromLocalStorage = ()=>{
    ...
    let users = [];
    users = Array.isArray(usersFromLocalStorage) && usersFromLocalStorage.length > 1 ? usersFromLocalStorage : [
...
function onClickSubmitSignIn(e) {
    ...
    if (email === "" || password === "") return (0, _notiflixDefault.default).Notify.failure("Please fill in all fields");
    let users = (0, _localStorage.getUserFromLocalStorage)();
    //if localStorage is empty
    if (!users[0]) return (0, _notiflixDefault.default).Notify.failure(`We dont have user with name : ${email}`);
    //is valid email?
    const filterUserByEmail = users.filter((el)=>el.email === email);
    if (filterUserByEmail.length === 0) return (0, _notiflixDefault.default).Notify.failure(`We dont have user with name : ${email}`);
    // //is valid password?
    if (filterUserByEmail.length >= 1 && filterUserByEmail[0].password !== password) return (0, _notiflixDefault.default).Notify.failure(`The password is not valid for user : ${email}`);
    //do we have user with this data?
    const isAvailableUser = users.some((el)=>el.email === email && el.password === password);
    return isAvailableUser ? (0, _notiflixDefault.default).Notify.success("Yeees,you are sign in. Here should be next step") : null;
}

},{"./localStorage":"45bAM","notiflix":"5z0Oc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"45bAM":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getUserFromLocalStorage", ()=>getUserFromLocalStorage);
const getUserFromLocalStorage = ()=>{
...
function setUserToLocalStorage(user) {
    ...
    if (isIncludeUser) (0, _notiflixDefault.default).Notify.warning("Sorry, but this user already exists");
    else {
        users.push(user);
...
},{"./localStorage":"45bAM","notiflix":"5z0Oc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["igKGj","8lqZg"], "8lqZg", "parcelRequire00cd")
